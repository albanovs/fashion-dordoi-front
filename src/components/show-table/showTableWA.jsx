...
export default function ShowTableWA() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(url + '/test/whatsappSlot');
    ...
    return (
        <div className="">
            <h1 className="title m-5 flex items-center justify-center">
                <div className='flex flex-col justify-center items-center'><BsWhatsapp size={50} />Whatsapp слот</div>
            </h1>
            ...
            <table className='border border-collapse text-center mb-10'>
                <thead className='bg-fuchsia-950'>
                    <tr>
                        <th className='lg:text-[14px] text-[8px] text-white p-2 border'><BsWhatsapp /></th>
                ...
                <tbody className='lg:text-sm text-[6px]'>
                    {
                        responseData.map(elem => (
                            <tr key={elem._id} data-elem={elem} className='border'>
                                {elem.num === 1 && <th rowSpan='20' className='bg-sky-100/40  pl-2 pr-2 text-[6px] lg:text-[10px]'>{elem.account}</th>}
