...
const EditableTable = () => {
    ...
    const addBtn = async (e) => {
        ...
        if (req) {
            try {
                const response = await axios.post(url + '/insert/account', {
                    account: req
                });
            ...
            <form onSubmit={handleSubmit}>
                <table className='border border-collapse text-center mb-10'>
                    <thead className='bg-fuchsia-950'>
                        <tr>
                            <th className='lg:text-[14px] text-[8px] text-white p-2 border'><BsInstagram /></th>
                    ...
                    <tbody className='lg:text-sm text-[8px]'>
                        {
                            responseData.map(elem => (
                                <tr key={elem._id} data-elem={elem} className='border'>
                                    {elem.num === 1 && <th rowSpan='5' className='pl-2 pr-2 text-[6px] lg:text-[10px]'>{elem.account}</th>}
