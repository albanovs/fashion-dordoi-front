...
const EditableWA = () => {
        ...
        try {
            await Promise.all(
                responseData.map((elem) =>
                    axios.patch(`${url}/test/whatsappSlot/${elem._id}`, {
    ...
    const addBtn = async (e) => {
        var req = prompt("Напишите whatsapp группу");

        if (req) {
            try {
                const response = await axios.post(url + '/slot/whatsapp', {
                    account: req
                });
